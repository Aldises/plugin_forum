...
*/
	...
	//	Second look for a localized file
        ...
        $result = mysql_fetch_assoc(
            mysql_query(
                "SELECT lang FROM mdl_course WHERE id='".$courseid."'",
                $qa_db_connection
            )
        );
        //set language code
...
*/
	{//Get the Q2A category tags from the URL
			...
			case QA_URL_FORMAT_PARAMS:
				if (!empty($request))
					foreach ($requestparts as $partindex => $requestpart)
						$paramsextra.=(strlen($paramsextra) ? '&' : '?').'k_1='.$category_tags.'&qa'.($partindex ? ('_'.$partindex) : '').'='.$requestpart;
				break;
		}
		...
		//Mis en commentaire car le # emp阠he l'utilisateur de cliquer sur les liens de la navigation et sur les tags et puis 鏰 sert ?rien
